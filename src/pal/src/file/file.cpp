        ...
        IN HANDLE hTemplateFile)
{
    ...
    if (lpFileName != NULL)
    {
        length = PAL_wcslen(lpFileName);
    }
    
    name = namePathString.OpenStringBuffer(length);
    size = WideCharToMultiByte( CP_ACP, 0, lpFileName, -1, name, length+1,
                                NULL, NULL );
      ...
      IN BOOL bFailIfExists)
{
    ...
    {
        length = PAL_wcslen(lpExistingFileName);
    }
    
    source = sourcePathString.OpenStringBuffer(length);
    src_size = WideCharToMultiByte( CP_ACP, 0, lpExistingFileName, -1, source, length+1,
                                NULL, NULL );
    ...
    {
        length = PAL_wcslen(lpNewFileName);
    }
    
    dest = destPathString.OpenStringBuffer(length);
    dest_size = WideCharToMultiByte( CP_ACP, 0, lpNewFileName, -1, dest, length+1,
                                NULL, NULL );
           ...
           IN LPCWSTR lpFileName)
{
    ...
    size = WideCharToMultiByte( CP_ACP, 0, lpFileName, -1, filename, length+1,
                                NULL, NULL );
                 ...
                 OUT LPSTR lpTempFileName)
{
    ...
    if ( length + MAX_SEEDSIZE + MAX_PREFIX >= MAX_LONGPATH ) 
    {
    ...
    *file_template = '\0';
    ...
    length = file_templatePS.GetCount();
         ...
         OUT LPWSTR lpTempFileName)
{
    ...
    {
        length = PAL_wcslen(lpPrefixString);
