...
set _buildNative=1
set _buildTests=1
set _buildPackages=1
set _buildNativeCoreLib=1
...
if /i "%1" == "freebsdmscorlib"     (set _buildNativeCoreLib=0&set _buildNative=0&set _buildTests=0&set _buildPackages=0&set __BuildOS=FreeBSD&set _skipNugetPackage=1&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "linuxmscorlib"       (set _buildNativeCoreLib=0&set _buildNative=0&set _buildTests=0&set _buildPackages=0&set __BuildOS=Linux&set _skipNugetPackage=1&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "netbsdmscorlib"      (set _buildNativeCoreLib=0&set _buildNative=0&set _buildTests=0&set _buildPackages=0&set __BuildOS=NetBSD&set _skipNugetPackage=1&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "osxmscorlib"         (set _buildNativeCoreLib=0&set _buildNative=0&set _buildTests=0&set _buildPackages=0&set __BuildOS=OSX&set _skipNugetPackage=1&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "windowsmscorlib"     (set _buildNativeCoreLib=0&set _buildNative=0&set _buildTests=0&set _buildPackages=0&set __BuildOS=Windows_NT&set _skipNugetPackage=1&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "nativemscorlib"      (set _buildNativeCoreLib=1&set _buildCoreLib=0&set _buildNative=0&set _buildTests=0&set _buildPackages=0&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "configureonly"       (set _configureOnly=1&set _buildNative=1&set _buildNativeCoreLib=0&set _buildCoreLib=0&set _buildTests=0&set _buildPackages=0&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "skipconfigure"       (set _skipConfigure=1&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "skipmscorlib"        (set _buildCoreLib=0&set _buildNativeCoreLib=0&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "skipnative"          (set _buildNative=0&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "skiptests"           (set _buildTests=0&set processedArgs=!processedArgs! %1&goto Arg_Next)
if /i "%1" == "skipbuildpackages"   (set _buildPackages=0&set processedArgs=!processedArgs! %1&goto Arg_Next)
...
if %_buildNative% EQU 1 ( 
    ...
    echo %__MsgPrefix%Using environment: "%VS140COMNTOOLS%\..\..\VC\vcvarsall.bat" %__VCBuildArch%
...
:GenVSSolution
    if defined _skipConfigure goto SkipConfigure
    ...
    popd
:SkipConfigure
    if defined _configureOnly goto SkipNativeBuild
...
if /i "%__BuildArch%"=="arm64" (
    ...
    if defined _skipConfigure goto SkipConfigureCrossBuild
...
:SkipConfigureCrossBuild
    ...
    if defined _configureOnly goto SkipCrossCompBuild
...
if %_buildCoreLib% EQU 1 (  
    ...
    echo %__MsgPrefix%Using environment: "%VS140COMNTOOLS%\VsDevCmd.bat"
...
if %_buildTests% EQU 1 (
    ...
    set __BuildtestArgs=%__BuildArch% %__BuildType% %__runArgs% %__unprocessedBuildArgs%

    rem arm64 builds currently use private toolset which has not been released yet
    REM TODO, remove once the toolset is open.
    if /i "%__BuildArch%" == "arm64" call :PrivateToolSet 

    call %__ProjectDir%\build-test.cmd %__BuildtestArgs%
